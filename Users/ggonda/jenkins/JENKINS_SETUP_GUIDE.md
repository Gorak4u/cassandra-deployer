...
3.  You will see the parameters defined in the `Jenkinsfile`:
    ...
    *   `NODE_SELECTION_METHOD`: A dropdown to choose between targeting nodes via a `QV_QUERY` or a static `NODE_LIST`.
    *   `QV_QUERY`, `NODE_LIST`, etc.: Text fields for the operation's arguments. Fill these in based on your chosen node selection method.
