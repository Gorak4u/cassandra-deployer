...
usage() {
    ...
    log_message "  --nodetool-options <\"opts\">  Pass additional raw options to the nodetool command (e.g., \"-g CELL -j 2\")."
...
# We can't just pass NODETOOL_EXTRA_OPTS in quotes because it may contain multiple flags.
# We must let the shell parse it. We disable globbing to be safe.
set -f
CMD="nodetool garbagecollect ${NODETOOL_EXTRA_OPTS}"
set +f
...
# Execute the command
# Use eval to correctly handle the options string
if eval "$CMD"; then
