...
usage() {
    ...
    printf "  %-30s %s\n" "--old-dc-nodes <list>" "A comma-separated list of nodes for the existing datacenter."
    printf "  %-30s %s\n" "--new-dc-nodes <list>" "A comma-separated list of nodes for the new datacenter."
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --old-dc-nodes) OLD_DC_NODES="$2"; shift ;;
        --new-dc-nodes) NEW_DC_NODES="$2"; shift ;;
...
# --- Validation ---
if [ -z "$OLD_DC_NAME" ] || [ -z "$NEW_DC_NAME" ]; then
    log_error "Missing datacenter names (--old-dc-name, --new-dc-name)."
    usage; exit 1
fi
if { [ -z "$OLD_DC_QUERY" ] && [ -z "$OLD_DC_NODES" ]; } || { [ -z "$NEW_DC_QUERY" ] && [ -z "$NEW_DC_NODES" ]; }; then
    log_error "Must specify a node source for both old and new datacenters (e.g., --old-dc-query or --old-dc-nodes)."
    usage; exit 1
fi
if [ -n "$OLD_DC_QUERY" ] && [ -n "$OLD_DC_NODES" ]; then
    log_error "Cannot use --old-dc-query and --old-dc-nodes together. Please choose one."; usage; exit 1
fi
if [ -n "$NEW_DC_QUERY" ] && [ -n "$NEW_DC_NODES" ]; then
    log_error "Cannot use --new-dc-query and --new-dc-nodes together. Please choose one."; usage; exit 1
fi
...
if [ -n "$OLD_DC_QUERY" ] || [ -n "$NEW_DC_QUERY" ]; then
...
if [ -n "$OLD_DC_QUERY" ]; then
    OLD_NODES_ARRAY=($(qv -t "$OLD_DC_QUERY"))
else
    IFS=',' read -r -a OLD_NODES_ARRAY <<< "$OLD_DC_NODES"
fi

if [ -n "$NEW_DC_QUERY" ]; then
    NEW_NODES_ARRAY=($(qv -t "$NEW_DC_QUERY"))
else
    IFS=',' read -r -a NEW_NODES_ARRAY <<< "$NEW_DC_NODES"
fi


if [ ${#OLD_NODES_ARRAY[@]} -eq 0 ] || [ ${#NEW_NODES_ARRAY[@]} -eq 0 ]; then
    log_error "Could not discover nodes for one or both datacenters. Aborting."
    exit 1
fi

log_info "Discovered ${#OLD_NODES_ARRAY[@]} nodes in old DC and ${#NEW_NODES_ARRAY[@]} in new DC."
OLD_DC_RUN_NODE=${OLD_NODES_ARRAY[0]}
NEW_DC_RUN_NODE=${NEW_NODES_ARRAY[0]}
...
OLD_DC_NODES_ARG=$([ -n "$OLD_DC_QUERY" ] && echo "--qv-query \"$OLD_DC_QUERY\"" || echo "--nodes \"$OLD_DC_NODES\"")
NEW_DC_NODES_ARG=$([ -n "$NEW_DC_QUERY" ] && echo "--qv-query \"$NEW_DC_QUERY\"" || echo "--nodes \"$NEW_DC_NODES\"")
...
run_cassy $OLD_DC_NODES_ARG -P -c "sudo /usr/local/bin/cass-ops disable-automation '${disable_reason}'"
log_info "Disabling automation on NEW datacenter nodes..."
run_cassy $NEW_DC_NODES_ARG -P -c "sudo /usr/local/bin/cass-ops disable-automation '${disable_reason}'"
...
run_cassy --rolling-op restart $NEW_DC_NODES_ARG
...
run_cassy $NEW_DC_NODES_ARG -c "$REBUILD_CMD"

log_success "Data rebuild process completed for the new datacenter."
log_info "--- Multi-DC Join Process Finished ---"
log_info "Run '$CASSY_SCRIPT_PATH $OLD_DC_NODES_ARG -c \"nodetool status\"' to verify cluster state."
...
log_info "  ${CYAN}${CASSY_SCRIPT_PATH} ${OLD_DC_NODES_ARG} -P -c \"sudo /usr/local/bin/cass-ops enable-automation\"${NC}"
log_info "  ${CYAN}${CASSY_SCRIPT_PATH} ${NEW_DC_NODES_ARG} -P -c \"sudo /usr/local/bin/cass-ops enable-automation\"${NC}"
