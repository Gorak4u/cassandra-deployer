...
usage() {
    ...
    printf "  %-30s %s\n" "--nodes <list>" "A comma-separated list of all nodes in the cluster."
...
if [ -z "$OLD_NAME" ] || [ -z "$NEW_NAME" ]; then
    log_error "Missing --old-name or --new-name."; usage; exit 1
fi
if [ -z "$QV_QUERY" ] && [ -z "$NODES_LIST" ]; then
    log_error "Missing node source. Use --qv-query or --nodes."; usage; exit 1
fi
if [ -n "$QV_QUERY" ] && [ -n "$NODES_LIST" ]; then
    log_error "Specify either --qv-query or --nodes, but not both."; usage; exit 1
fi
...
if [ -n "$QV_QUERY" ] && ! command -v qv &> /dev/null; then
    log_error "'qv' command not found, which is required for --qv-query."
    exit 1
fi
...
if [ ${#NODES[@]} -eq 0 ]; then
    log_error "Could not discover any nodes. Aborting."
    exit 1
fi
...
# Determine the correct node selection argument for cassy.sh
NODE_SELECTION_FLAG=""
NODE_SELECTION_VALUE=""
if [ -n "$QV_QUERY" ]; then
    NODE_SELECTION_FLAG="--qv-query"
    NODE_SELECTION_VALUE="$QV_QUERY"
else
    NODE_SELECTION_FLAG="--nodes"
    NODE_SELECTION_VALUE="$NODES_LIST"
fi

# --- Disable Automation ---
log_info "--- Disabling Automation on all nodes ---"
disable_reason="Pausing automation for cluster rename from '${OLD_NAME}' to '${NEW_NAME}'"
run_cassy "$NODE_SELECTION_FLAG" "$NODE_SELECTION_VALUE" -P -c "sudo /usr/local/bin/cass-ops disable-automation '${disable_reason}'"
...
run_cassy "$NODE_SELECTION_FLAG" "$NODE_SELECTION_VALUE" -P -c "$UPDATE_CQL_CMD"
log_success "system.local updated on all nodes."

# --- Phase 3: Stop Cluster ---
log_info "--- Phase 3: Stopping all Cassandra nodes ---"
run_cassy "$NODE_SELECTION_FLAG" "$NODE_SELECTION_VALUE" -P -c 'sudo /usr/local/bin/cass-ops stop'
...
run_cassy "$NODE_SELECTION_FLAG" "$NODE_SELECTION_VALUE" -P -c "$SED_CMD"
log_success "cassandra.yaml updated on all nodes."

# --- Phase 5: Start Cluster ---
log_info "--- Phase 5: Starting all Cassandra nodes ---"
run_cassy "$NODE_SELECTION_FLAG" "$NODE_SELECTION_VALUE" -P -c 'sudo systemctl start cassandra'
...
log_info "  ${CYAN}${CASSY_SCRIPT_PATH} ${NODE_SELECTION_FLAG} \"${NODE_SELECTION_VALUE}\" -P -c \"sudo /usr/local/bin/cass-ops enable-automation\"${NC}"
log_info "Then run Puppet on all nodes to make the configuration change permanent."
log_info "Monitor cluster health with: ${CASSY_SCRIPT_PATH} ${NODE_SELECTION_FLAG} \"${NODE_SELECTION_VALUE}\" -c 'nodetool status'"
