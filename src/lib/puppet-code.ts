...
export const puppetCode = {
  ...
  manifests: {
    ...
    'config.pp': [
      ...
      '  file { [$cassandra::data_dir, $cassandra::commitlog_dir]:',
      '    ensure  => \'directory\',',
  ...
  templates: {
    'cassandra.yaml.erb': [
      ...
      'rpc_address: <%= @cassandra.listen_address %>',
      'seed_provider:',
      ...
      '  - <%= @cassandra.data_dir %>',
      'commitlog_directory: <%= @cassandra.commitlog_dir %>',
      'saved_caches_directory: /var/lib/cassandra/saved_caches',
      'authenticator: PasswordAuthenticator',
      'authorizer: CassandraAuthorizer',
      '<% if @cassandra.ssl_enabled == true -%>',
      'server_encryption_options:',
      '  internode_encryption: <%= @cassandra.internode_encryption %>',
      '  keystore: <%= @cassandra.keystore_path %>',
      '  keystore_password: <%= @cassandra.keystore_password %>',
      '  truststore: <%= @cassandra.truststore_path %>',
      ...
      '  keystore_password: <%= @cassandra.keystore_password %>',
      '<% end -%>',
      '<% if @cassandra.replace_address != "" -%>',
      'replace_address_first_boot: <%= @cassandra.replace_address %>',
      '<% end -%>',
      'concurrent_reads: 16',
    ...
    'jvm-server.options.erb': [
      ...
      '# This file is for Cassandra 4.x and later, with intelligent version detection.',
      '',
      '# Common settings for all versions',
      '-Xms<%= @cassandra.max_heap_size %>',
      '-Xmx<%= @cassandra.max_heap_size %>',
      '-XX:+HeapDumpOnOutOfMemoryError',
      '-Dcassandra.jmx.local.port=7199',
      '',
      '<%# Logic for Java 11 and newer -%>',
      '<% if @cassandra.java_version.to_i >= 11 -%>',
      '# Settings for Java 11+',
      '<% if @cassandra.gc_type == "G1GC" -%>',
      '-XX:+UseG1GC',
      '-XX:G1RSetUpdatingPauseTimePercent=5',
      '-XX:MaxGCPauseMillis=500',
      '<% elsif @cassandra.gc_type == "Shenandoah" -%>',
      '-XX:+UseShenandoahGC',
      '<% end -%>',
      '',
      '<%# Logic for Java 8 -%>',
    ...
    'cqlshrc.erb': [
      '[authentication]',
      'username = cassandra',
      'password = <%= @cassandra.cassandra_password %>',
    ].join('\n'),
