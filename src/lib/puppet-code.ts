...
server_encryption_options:
  ...
  cipher_suites: [TLS_RSA_WITH_AES_128_CBC_SHA, TLS_DHE_RSA_WITH_AES_256_GCM_SHA384]
  protocol: <%= @tls_protocol %>
  algorithm: <%= @tls_algorithm %>
  store_type: <%= @store_type %>

client_encryption_options:
  enabled: <%= @client_optional || @client_require_client_auth %>
...
port = 9042
[ssl]
certfile = /etc/cassandra/cassandra.cert
userkey = /etc/cassandra/cassandra.key
usercert = /etc/cassandra/cassandra.crt
        `.trim(),
...
class profile_cassandra_pfpt {
  ...
  $cqlsh_path_env                   = lookup('profile_ggonda_cassandr::cqlsh_path_env', { 'default_value' => '/usr/bin/' })
  $jamm_target                      = lookup('profile_ggonda_cassandr::jamm_target', { 'default_value' => '/usr/share/cassandra/lib/jamm-0.3.2.jar' })
  $jamm_source                      = lookup('profile_ggonda_cassandr::jamm_source', { 'default_value' => 'puppet:///modules/cassandra_pfpt/files/jamm-0.3.2.jar' })
