...
*   `profile_cassandra_pfpt::jvm_overrides_from_hiera` (Hash): A hash of extra JVM arguments to add or override in `jvm-server.options`. Default: `{}`.
...
*   `profile_cassandra_pfpt::disable_swap` (Boolean): If true, will disable swap and comment it out in `/etc/fstab`. Default: `false`.
