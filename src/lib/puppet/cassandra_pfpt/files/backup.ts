...
do_schema_restore() {
    ...
    if aws s3 cp "\$S3_PATH" - | tar -xzf - --absolute-names --to-stdout schema.cql > /tmp/schema.cql 2>/dev/null; then
...
MANIFEST_JSON=\$(aws s3 cp "\$S3_PATH" - | tar -xzf - --absolute-names --to-stdout backup_manifest.json 2>/dev/null)
