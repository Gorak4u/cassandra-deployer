...
do_schema_restore() {
    ...
    local RESTORE_TEMP_DIR="/tmp/schema_restore_\${BACKUP_ID}"
    trap 'rm -rf "\${RESTORE_TEMP_DIR}"' EXIT
    mkdir -p "\${RESTORE_TEMP_DIR}"

    log_message "1. Downloading backup to temporary location..."
    # In a real environment, you'd uncomment the aws command
    # touch "\${RESTORE_TEMP_DIR}/\${TARBALL_NAME}" # Mock download
    aws s3 cp "\${S3_PATH}" "\${RESTORE_TEMP_DIR}/\${TARBALL_NAME}"

    log_message "2. Extracting schema.cql from archive..."
    if tar -xzf "\${RESTORE_TEMP_DIR}/\${TARBALL_NAME}" -C /tmp/ schema.cql; then
