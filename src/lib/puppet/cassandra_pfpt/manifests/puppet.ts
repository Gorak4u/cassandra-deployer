...
class cassandra_pfpt::puppet inherits cassandra_pfpt {
  ...
  if $puppet_cron_schedule and $puppet_cron_schedule != '' {
    ...
    if $minute_field =~ /^\\d+(,\\d+)*$/ {
      # The cron resource can take an array of numbers, but splitting returns strings.
      # It's simpler and more robust to just pass the string if it's a valid list.
      $minute = $minute_field
    } else {
      # Handle other cron expressions like '*/15'
      $minute = $minute_field
    }
