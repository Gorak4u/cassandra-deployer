...
class cassandra_pfpt::service inherits cassandra_pfpt {
  ...
  # Change the default password, but only if the new password isn't already active.
  exec { 'change_cassandra_password':
    ...
    # This check is idempotent. It succeeds if we can already log in with the new password.
