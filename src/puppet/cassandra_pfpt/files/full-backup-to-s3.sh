...
process_table_backup() {
    ...
    if [ "$BACKUP_BACKEND" == "s3" ]; then
        ...
        if [ "$UPLOAD_STREAMING" = "true" ]; then
            ...
            nice -n 19 ionice -c 3 env $THROTTLE_PREFIX aws s3 cp --quiet - "$s3_path" "${object_lock_flags[@]}"
            ...
            if ! nice -n 19 ionice -c 3 env $THROTTLE_PREFIX aws s3 cp --quiet "$local_enc_file" "$s3_path" "${object_lock_flags[@]}"; then
...
if [ -f "/var/lib/upload-disabled" ]; then
    ...
    if [ "$BACKUP_BACKEND" == "s3" ]; then
        ...
        if ! env $THROTTLE_PREFIX aws s3 cp --quiet "$MANIFEST_FILE" "$MANIFEST_S3_PATH" "${object_lock_flags[@]}"; then
        ...
        if ! env $THROTTLE_PREFIX aws s3 cp --quiet "$SCHEMA_MAP_FILE" "$SCHEMA_MAP_S3_PATH" "${object_lock_flags[@]}"; then
        ...
        if [ -f "$SCHEMA_DUMP_FILE" ]; then
            ...
            if ! env $THROTTLE_PREFIX aws s3 cp --quiet "$SCHEMA_DUMP_FILE" "$SCHEMA_S3_PATH" "${object_lock_flags[@]}"; then
