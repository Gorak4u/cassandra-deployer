...
cleanup_old_snapshots() {
    ...
    # Find all unique snapshot directories across all keyspaces/tables. -prune prevents find from descending into the snapshot dirs themselves.
    ...
    # Get just the unique snapshot tag names (the final directory component)
    local unique_tags
    ...
    for tag in $unique_tags; do
      ...
      # If we successfully parsed a date string, convert it to a UNIX timestamp
