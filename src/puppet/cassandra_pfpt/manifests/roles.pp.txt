  ...
  if !empty($cassandra_roles) {
    $cassandra_roles.each |$role_name, $role_details| {
      ...
      exec { "create_cassandra_role_${role_name}":
        command => "cqlsh ${cqlsh_ssl_opt} -u cassandra -p '${cassandra_password}' -f ${cql_file_path}",
        ...
        unless  => "cqlsh ${cqlsh_ssl_opt} -u cassandra -p '${cassandra_password}' -e \"DESCRIBE ROLE \"${role_name}\"\";",
