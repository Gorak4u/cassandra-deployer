...
# It is recommended to use 'cassy.sh --rolling-op reboot' directly.
#
# All arguments passed to this script will be forwarded to cassy.sh.
#
# Example:
#   ./scripts/rolling_reboot.sh --qv-query "-r role_cassandra_pfpt -d AWSLAB"

set -euo pipefail

echo "INFO: Using legacy wrapper. It is recommended to use 'cassy.sh --rolling-op reboot' directly." >&2

# Get the directory where this script is located
SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"

# Execute the main cassy.sh script with the rolling-op and pass all other arguments through
exec "${SCRIPT_DIR}/cassy.sh" --rolling-op reboot "$@"
