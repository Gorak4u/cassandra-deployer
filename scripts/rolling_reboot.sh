#!/bin/bash
# Performs a safe, rolling reboot of Cassandra nodes.
...
usage() {
    ...
    echo "This will perform a rolling reboot on all nodes returned by the qv query,"
...
echo "Starting a safe rolling reboot for nodes matching query: '${QV_QUERY}'"
echo "A health check will be performed after each node reboots."

./scripts/cassy.sh --qv-query "${QV_QUERY}" \
  -c "sudo /usr/local/bin/cass-ops reboot" \
  --inter-node-check "${HEALTH_CHECK_SCRIPT}"

echo "Rolling reboot process completed."
